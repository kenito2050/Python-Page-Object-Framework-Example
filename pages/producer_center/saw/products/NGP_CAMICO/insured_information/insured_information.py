from selenium.webdriver.common.by import By

class Insured_Information():

    def __init__(self, driver):
        self.driver = driver

    def PageElements(self):
        self.annual_revenue = self.driver.find_element(By.ID, "annual_revenue_current_year")

        self.cpa_count = self.driver.find_element(By.ID, "cpa_count")

        self.next_button = self.driver.find_element(By.NAME, "submit")

        self.return_to_Admin = self.driver.find_element(By.LINK_TEXT, "Return to Admin Interface")

        return self

    def enter_annual_revenue(self, revenue):
        Insured_Information.PageElements(self).annual_revenue.click()
        Insured_Information.PageElements(self).annual_revenue.clear()
        Insured_Information.PageElements(self).annual_revenue.send_keys(revenue)

    def enter_cpa_count(self, cpa_count):
        Insured_Information.PageElements(self).cpa_count.click()
        Insured_Information.PageElements(self).cpa_count.send_keys(cpa_count)

    def click_next(self):
        Insured_Information.PageElements(self).next_button.click()

    def click_return_to_Admin_Interface(self):
        Insured_Information.PageElements(self).return_to_Admin.click()

        # return_to_admin_interface = self.driver.find_element(By.LINK_TEXT, "Return to Admin Interface")
        # return_to_admin_interface.click()