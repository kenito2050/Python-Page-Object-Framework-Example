...
from pages.producer_center.saw.products.CYB_MDA.PAF.PAF import PAF

### Generic Coverage Options Classes
from pages.producer_center.saw.products.CYB_MDA.coverage_options.coverage_options import Coverage_Options
...
class CreateQuote():
    ...
    def test_login_search_for_agent_create_quote(self):
        ...
        for x in range(1, sh.nrows):
            ...
            if test_scenario == "1":
                saw_PAF.create_quote_No_DQ_corporation(broker, staff_count)
            elif test_scenario == "2":
                saw_PAF.create_quote_No_DQ_Partnership(broker, staff_count)
            elif test_scenario == "3":
                saw_PAF.create_quote_No_DQ_LLC(broker, staff_count)
            elif test_scenario == "4":
                saw_PAF.create_quote_No_DQ_other(broker, staff_count)
