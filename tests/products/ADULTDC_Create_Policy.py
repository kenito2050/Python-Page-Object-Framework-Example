...
from pages.producer_center.saw.products.ADULTDC.insured_information.insured_information import Insured_Information
from pages.producer_center.saw.products.ADULTDC.PAF.PAF import PAF
from pages.producer_center.saw.products.ADULTDC.coverage_options.HNOA_coverage_options import HNOA_Coverage_Options
from pages.producer_center.saw.products.ADULTDC.coverage_options.No_HNOA_coverage_options import No_HNOA_Coverage_Options
from pages.producer_center.saw.products.ADULTDC.coverage_options.coverage_options import Coverage_Options
from pages.producer_center.saw.products.ADULTDC.select_option.select_option import Select_Option
...
class TestCreateQuote():
    ...
    def test_login_search_for_agent_create_quote(self, browser, env):
        ...
        global state
        global zip
        global number_daily_participants
        ...
        for x in range(1, sh.nrows):
            ...
            if empty_cell == False:
                ...
                number_daily_participants = str(round(sh.cell_value(i, 7)))
                years_in_business = str(round(sh.cell_value(i, 8)))
                staff_count = str(round(sh.cell_value(i, 9)))
                _OLD_scenario = sh.cell_value(i, 10)
                _OLD_scenario_number = str(round(sh.cell_value(i, 11)))
        ...
        saw_ii.enter_number_daily_participants(number_daily_participants)
        saw_ii.click_next()
